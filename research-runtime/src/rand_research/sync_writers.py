...
from rand_research.io_utils import atomic_write_text, locked_update_json
from rand_research.models import SCHEMA_VERSION, NormalizedItem


def legacy_write_memx_journal(path: Path, run_id: str, preset: str, items: list[NormalizedItem], artifacts: dict[str, str]) -> dict[str, Any]:
...
def legacy_write_tracker_sync(path: Path, run_id: str, preset: str, items: list[NormalizedItem], gate_payload: dict[str, Any]) -> dict[str, Any]:
...
def write_memx_journal(
    ...
    locked_update_json(
        path,
        lambda: {"schema_version": SCHEMA_VERSION, "entries": []},
        append,
    )
    return entry


def write_tracker_sync(
    path: Path,
    run_id: str,
    preset: str,
    items: list[NormalizedItem],
    ...
    locked_update_json(
        path,
        lambda: {"schema_version": SCHEMA_VERSION, "events": []},
        append,
    )
    return event
